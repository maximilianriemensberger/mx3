...
bool WebSocket::del_listener(ListenerKey key) {
...
bool WebSocket::is_connected() { return web_socket_->is_connected(); }

void WebSocket::close() { web_socket_->close(); }

void WebSocket::send(const string & message) { web_socket_->send(message); }

BroadcastingWebSocketListener::BroadcastingWebSocketListener() : listeners_{} {}

ListenerKey BroadcastingWebSocketListener::add_listener(
        unique_ptr<mx3_gen::WebSocketListener> listener) {
    ListenerKey key = (ListenerKey)listener.get();
    listeners_.insert(std::make_pair(key, move(listener)));
    return key;
}

bool BroadcastingWebSocketListener::del_listener(ListenerKey key) {
    return listeners_.erase(key) > 0;
}

void BroadcastingWebSocketListener::on_open(int16_t code,
                                            const string & message) {
    for (const auto & listener : listeners_)
        listener.second->on_open(code, message);
}

void BroadcastingWebSocketListener::on_close(int16_t code,
                                             const string & message,
                                             bool remote) {
    for (const auto & listener : listeners_)
...
void BroadcastingWebSocketListener::on_error(int16_t code,
                                             const string & message) {
    for (const auto & listener : listeners_)
        listener.second->on_error(code, message);
}

void BroadcastingWebSocketListener::on_message(const string & message) {
    for (const auto & listener : listeners_)
...
void ForwardingWebSocketListener::on_open(int16_t /*code*/,
                                          const string & /*message*/) {}

void ForwardingWebSocketListener::on_close(int16_t /*code*/,
                                           const string & /*message*/,
                                           bool /*remote*/) {}

void ForwardingWebSocketListener::on_error(int16_t /*code*/,
                                           const string & /*message*/) {}

void ForwardingWebSocketListener::on_message(const string & message) {
...
void LoggingWebSocketListener::on_open(int16_t code, const string & message) {
    logger_->log(mx3_gen::Logger::LOG_LEVEL_INFO, "WebSocket",
                 "OPEN Code: " + to_string(code) + " Message: " + message);
}

void LoggingWebSocketListener::on_close(int16_t code, const string & message,
                                        bool /*remote*/) {
    logger_->log(mx3_gen::Logger::LOG_LEVEL_INFO, "WebSocket",
                 "CLOSE Code: " + to_string(code) + " Message: " + message);
}

void LoggingWebSocketListener::on_error(int16_t code, const string & message) {
    logger_->log(mx3_gen::Logger::LOG_LEVEL_ERROR, "WebSocket",
                 "ERROR Code: " + to_string(code) + " Message: " + message);
}

void LoggingWebSocketListener::on_message(const string & message) {
    logger_->log(mx3_gen::Logger::LOG_LEVEL_INFO, "WebSocket",
                 "MESSAGE: " + message);
}
