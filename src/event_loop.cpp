...
EventLoopRef::EventLoopRef(shared_ptr<mx3_gen::EventLoop> loop)
        : m_loop{std::move(loop)}
{
}

void EventLoopRef::post(const SingleThreadTaskRunner::Task & task)
{
    m_loop->post(make_shared<FnTask>(task));
}

EventLoopCpp::EventLoopCpp(const shared_ptr<mx3_gen::ThreadLauncher> & launcher)
        : m_stop(false), m_done(false)
{
    auto task = make_shared<FnTask>([this]() { _run_loop(); });
...
void EventLoopCpp::_run_loop()
{
    while (true) {
        std::unique_lock<std::mutex> lk(m_task_mutex);
        m_task_cv.wait(lk,
                       [this] { return m_stop == true || !m_queue.empty(); });
        if (m_stop == true) {
            break;
        }
