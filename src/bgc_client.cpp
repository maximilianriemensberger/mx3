...
shared_ptr<mx3_gen::BgcClient> mx3_gen::BgcClient::create_bgc_client(
        const string & root_path, const shared_ptr<EventLoop> & ui_thread_impl,
        ...
        const shared_ptr<MulticastSocket> & multicast_socket_impl)
{
    return make_shared<mx3::BgcClient>(
            ...
            logger_impl, http_impl);
}

BgcClient::BgcClient(const string & root_path,
        ...
        : root_path_{root_path}, ui_thread_{move(ui_runner)},
          ...
          http_{move(http_client)}, server_uri_{}, multicast_address_{{}, 0}
{
    ...
    multicast_socket_->add_listener(
